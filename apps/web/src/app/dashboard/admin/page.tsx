    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="border-b border-surface-border bg-surface-raised px-6 py-4">
          <h1 className="text-lg font-semibold text-content">Tenant Admin</h1>
          <p className="text-[12px] text-content-tertiary">StitchBoat Immigration · Branding, SSO, case types, agent policy, data retention</p>
        </div>

        <div className="flex">
          {/* Settings nav */}
          <div className="w-56 shrink-0 border-r border-surface-border bg-surface-raised p-3">
            {NAV.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-[13px] transition-colors ${
                    item.active ? 'bg-surface-sunken font-medium text-content' : 'text-content-tertiary hover:bg-surface hover:text-content-secondary'
                  }`}
                >
          ...
          <div className="flex-1 p-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-xl border border-surface-border bg-surface-raised p-5">
                <h3 className="text-[13px] font-semibold text-content">Branding</h3>
                <div className="mt-3 space-y-2 text-[13px]">
                  <div className="flex items-center justify-between">
                    <span className="text-content-tertiary">Custom domain</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-content-tertiary">DKIM / SPF</span>
                    <span className="flex items-center gap-1.5 text-emerald-600"><CheckCircle2 className="h-3.5 w-3.5" /> Verified</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-content-tertiary">Primary color</span>
                    <span className="flex items-center gap-2">
                      <span className="inline-block h-4 w-4 rounded bg-blue-600" />
                      <span className="font-mono text-content-secondary">#1d4ed8</span>
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-content-tertiary">Agent persona</span>
                    <span className="font-medium text-content-secondary">Neha</span>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-surface-border bg-surface-raised p-5">
                <div className="flex items-center justify-between">
                  <h3 className="text-[13px] font-semibold text-content">Case types enabled</h3>
                  <span className="text-[12px] text-content-muted">{CASE_TYPES.length} active</span>
                </div>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {CASE_TYPES.map((ct) => (
                    <span key={ct} className="rounded-md border border-surface-border bg-surface px-2 py-1 text-[12px] font-medium text-content-secondary">{ct}</span>
                  ))}
                </div>
              </div>
              ...
              {/* Agent policy */}
              <div className="rounded-xl border border-surface-border bg-surface-raised p-5">
                <h3 className="text-[13px] font-semibold text-content">Agent policy</h3>
              ...
              {/* Data & retention */}
              <div className="rounded-xl border border-surface-border bg-surface-raised p-5">
                <h3 className="text-[13px] font-semibold text-content">Data & retention</h3>
                <div className="mt-3 space-y-2 text-[13px]">
...
function DataRow({ label, value, mono }: { label: string; value: string; mono?: boolean }): ReactElement {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`font-medium ${mono ? 'font-mono text-[12px] text-content-secondary' : 'text-content-secondary'}`}>{value}</span>
    </div>
  );
}
