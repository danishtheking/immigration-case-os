...
const ALT_PATHS = [
  ...
  { type: 'EB-2 + PERM', score: null, label: 'Longer timeline', color: 'text-content-tertiary bg-surface-sunken' },
];
        ...
        <div className="grid grid-cols-12 gap-6 p-6">
          ...
          <div className="col-span-4 space-y-4">
            <div className="rounded-xl border border-surface-border bg-surface-raised p-6 text-center">
              <p className="text-[12px] font-medium uppercase tracking-wider text-content-muted">Overall fit score</p>
              <p className="mt-2 text-6xl font-black text-emerald-600">84</p>
              <p className="mt-1 text-[14px] font-semibold text-content-secondary">Strong</p>
              <p className="mt-2 text-[12px] text-content-muted">model v3.2 · updated 11 min ago</p>
              <div className="mt-4 flex justify-center gap-2">
                <button className="flex items-center gap-1 rounded-lg border border-surface-border px-3 py-1.5 text-[12px] text-content-secondary hover:bg-surface">
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-[12px] text-amber-800">
              ...
              <button className="mt-3 rounded-md bg-amber-600 px-3 py-1.5 text-[12px] font-semibold text-white hover:bg-amber-500">
                Approve assessment
              </button>
            </div>
            ...
            <div className="rounded-xl border border-surface-border bg-surface-raised p-5">
              <h3 className="text-[13px] font-semibold text-content">Alternate paths considered</h3>
              <ul className="mt-3 space-y-2">
                {ALT_PATHS.map((p) => (
                  <li key={p.type} className="flex items-center justify-between">
                    <span className="text-[13px] text-content-secondary">{p.type}</span>
                    <span className={`rounded-md px-2 py-0.5 text-[12px] font-semibold ${p.color}`}>
            ...
            {/* Summary */}
            <div className="rounded-xl border border-surface-border bg-surface-raised p-5">
              <h3 className="text-[13px] font-semibold text-content">Quick summary</h3>
              <div className="mt-3 grid grid-cols-3 gap-3 text-center">
                <div className="rounded-lg bg-emerald-50 p-3">
                  <p className="text-lg font-bold text-emerald-700">{metCount}</p>
                  <p className="text-[12px] text-emerald-600">Met</p>
                </div>
                <div className="rounded-lg bg-amber-50 p-3">
                  <p className="text-lg font-bold text-amber-700">1</p>
                  <p className="text-[12px] text-amber-600">Partial</p>
                </div>
                <div className="rounded-lg bg-surface-sunken p-3">
                  <p className="text-lg font-bold text-content-tertiary">0</p>
                  <p className="text-[12px] text-content-tertiary">Missing</p>
                </div>
              </div>
          ...
          <div className="col-span-8 space-y-3">
            {CRITERIA.map((c, i) => (
              <div key={c.code} className="rounded-xl border border-surface-border bg-surface-raised p-5 transition-colors hover:border-surface-border-hover">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-[13px] font-semibold text-content">
                        <span className="text-content-muted">#{i + 1}</span> {c.name}
                      </p>
                      <p className="mt-1.5 text-[12px] leading-relaxed text-content-secondary">{c.rationale}</p>
                    </div>
                  </div>
                  <span className={`shrink-0 rounded-md px-2.5 py-1 text-[12px] font-bold ${c.met ? 'bg-emerald-50 text-emerald-700' : c.confidence > 50 ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-600'}`}>
                    {c.confidence}%
                  </span>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {c.evidence.map((e) => (
                    <span key={e} className="flex items-center gap-1 rounded-md bg-blue-50 px-2 py-0.5 text-[12px] font-medium text-blue-700">
                      <FileText className="h-2.5 w-2.5" /> {e}
                    </span>
                  ))}
                  {c.gaps.map((g) => (
                    <span key={g} className="flex items-center gap-1 rounded-md bg-amber-50 px-2 py-0.5 text-[12px] font-medium text-amber-700">
