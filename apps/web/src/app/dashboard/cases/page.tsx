...
const DEMO_CASES = [
  ...
  { id: '5', name: 'Ayaan Hussein', detail: 'SOM · Asylum seeker', type: 'I-589', typeColor: 'bg-orange-50 text-orange-700', stage: 'Intake', stageColor: 'text-content-secondary', deadline: 'Apr 30', deadlineNote: 'Declaration', attorney: 'Danish', docs: 7 },
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-content">Cases</h1>
            <p className="text-[12px] text-content-tertiary">{filtered.length} of {DEMO_CASES.length} cases</p>
          </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-surface-border bg-surface-raised py-2 pl-9 pr-4 text-[13px] placeholder:text-content-muted focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-200"
            />
          </div>
          <button className="flex items-center gap-1.5 rounded-lg border border-surface-border bg-surface-raised px-3 py-2 text-[12px] text-content-secondary hover:bg-surface">
            ...
            <button
              ...
              className={`rounded-md px-2.5 py-1 text-[12px] font-medium transition-colors ${
                activeFilter === chip
                  ...
                  : 'border border-surface-border text-content-tertiary hover:bg-surface'
              }`}
            >
              {chip}
            </button>
            ...
            <thead>
              <tr className="border-b border-surface-border/50">
                <th className="px-4 py-3 text-left">
                  <input type="checkbox" className="rounded border-surface-border-hover" />
                </th>
                <th className="px-4 py-3 text-left text-[12px] font-semibold uppercase tracking-wider text-content-muted">
                  <button className="flex items-center gap-1">Client <ArrowUpDown className="h-3 w-3" /></button>
                </th>
                <th className="px-4 py-3 text-left text-[12px] font-semibold uppercase tracking-wider text-content-muted">Case type</th>
                <th className="px-4 py-3 text-left text-[12px] font-semibold uppercase tracking-wider text-content-muted">Stage</th>
                <th className="px-4 py-3 text-left text-[12px] font-semibold uppercase tracking-wider text-content-muted">Next deadline</th>
                <th className="px-4 py-3 text-left text-[12px] font-semibold uppercase tracking-wider text-content-muted">Attorney</th>
                <th className="px-4 py-3 text-left text-[12px] font-semibold uppercase tracking-wider text-content-muted">Docs</th>
                <th className="px-4 py-3 text-right text-[12px] font-semibold uppercase tracking-wider text-content-muted">Score</th>
                <th className="w-8"></th>
              </tr>
            </thead>
            <tbody className="text-[13px]">
              {filtered.map((c) => (
                <tr key={c.id} className="group border-b border-surface-border/30 transition-colors hover:bg-surface/60">
                  <td className="px-4 py-3"><input type="checkbox" className="rounded border-surface-border-hover" /></td>
                  <td className="px-4 py-3">
                    <Link href={`/dashboard/cases/${c.id}`} className="block">
                      <div className="font-medium text-content group-hover:text-blue-700">{c.name}</div>
                      <div className="text-[12px] text-content-muted">{c.detail}</div>
                    </Link>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex rounded-md px-2 py-0.5 text-[12px] font-semibold ${c.typeColor}`}>{c.type}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <Circle className={`h-2 w-2 fill-current ${c.stageColor}`} />
                      <span className="text-content-secondary">{c.stage}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`font-medium ${c.urgent ? 'text-red-600' : 'text-content-secondary'}`}>{c.deadline}</span>
                    <span className="ml-1.5 text-[12px] text-content-muted">{c.deadlineNote}</span>
                  </td>
                  <td className="px-4 py-3 text-content-secondary">{c.attorney}</td>
                  <td className="px-4 py-3 text-content-tertiary">{c.docs}</td>
                  ...
                  <td className="px-4 py-3 text-content-muted opacity-0 group-hover:opacity-100">
        ...
        <p className="mt-4 text-center text-[12px] text-content-muted">
