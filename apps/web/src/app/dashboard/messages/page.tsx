...
const CHANNEL_ICONS = {
  ...
  in_app: { icon: MessageSquare, color: 'text-content-secondary', bg: 'bg-surface-sunken' },
};
  ...
  return (
    <>
      ...
      <main className="flex flex-1 overflow-hidden">
        {/* Conversation list */}
        <div className="flex w-80 shrink-0 flex-col border-r border-surface-border bg-surface-raised">
          <div className="border-b border-surface-border/50 p-3">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-[14px] font-semibold text-content">Messages</h2>
              <div className="flex items-center gap-1">
                <span className="grid h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1.5 text-[12px] font-bold text-white">4</span>
              </div>
            </div>
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-surface-border bg-surface py-2 pl-8 pr-3 text-[12px] placeholder:text-content-muted focus:border-blue-500 focus:bg-surface-raised focus:outline-none"
              />
            </div>
            {/* Channel filter */}
            <div className="mt-2 flex gap-1">
              {(['all', 'whatsapp', 'email', 'sms'] as const).map((ch) => (
                <button key={ch} className={`rounded-md px-2 py-1 text-[12px] font-medium ${ch === 'all' ? 'bg-content text-white' : 'border border-surface-border text-content-tertiary hover:bg-surface'}`}>
            ...
            {filteredConversations.map((conv) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-start gap-3 border-b border-surface-border/30 px-3 py-3 text-left transition-colors ${
                    selectedId === conv.id ? 'bg-blue-50/60' : 'hover:bg-surface'
                  }`}
                >
                  <div className="relative">
                    <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-surface-sunken text-[12px] font-bold text-content-secondary">
                      {conv.avatar}
                    </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <p className="truncate text-[13px] font-semibold text-content">{conv.name}</p>
                      <span className="shrink-0 text-[12px] text-content-muted">{conv.time}</span>
                    </div>
                    <p className="text-[12px] text-content-muted">{conv.caseType}</p>
                    <div className="mt-0.5 flex items-center gap-1.5">
                      <Icon className={`h-3 w-3 shrink-0 ${ch.color}`} />
                      <p className="truncate text-[12px] text-content-tertiary">{conv.lastMessage}</p>
                    </div>
                  </div>
                  {conv.unread > 0 && (
                    <span className="mt-1 grid h-5 min-w-5 shrink-0 place-items-center rounded-full bg-blue-600 px-1 text-[12px] font-bold text-white">
                      {conv.unread}
                    </span>
                  )}
                </button>
        ...
        <div className="flex flex-1 flex-col">
          ...
          <div className="flex items-center justify-between border-b border-surface-border bg-surface-raised px-5 py-3">
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="grid h-9 w-9 place-items-center rounded-full bg-surface-sunken text-[12px] font-bold text-content-secondary">{selected.avatar}</div>
              ...
              <div>
                <p className="text-[13px] font-semibold text-content">{selected.name}</p>
            ...
            <div className="flex items-center gap-1">
              <button className="rounded-md p-2 text-content-muted hover:bg-surface-sunken hover:text-content-secondary"><Phone className="h-4 w-4" /></button>
              <button className="rounded-md p-2 text-content-muted hover:bg-surface-sunken hover:text-content-secondary"><Video className="h-4 w-4" /></button>
              <button className="rounded-md p-2 text-content-muted hover:bg-surface-sunken hover:text-content-secondary"><MoreHorizontal className="h-4 w-4" /></button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4">
            <div className="mb-4 text-center">
              <span className="rounded-full bg-surface-sunken px-3 py-1 text-[12px] text-content-tertiary">Today</span>
            </div>
              ...
              <div key={msg.id} className={`mb-3 flex ${msg.sender === 'firm' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${
                  msg.sender === 'firm'
                    ...
                    : 'rounded-bl-md bg-surface-sunken text-content'
                }`}>
                  {msg.sender === 'firm' && (
                    <p className={`mb-0.5 text-[12px] ${msg.sender === 'firm' ? 'text-blue-200' : 'text-content-muted'}`}>{msg.senderName}</p>
                  )}
                  <p className="text-[13px] leading-relaxed">{msg.content}</p>
                  <div className={`mt-1 flex items-center justify-end gap-1 text-[12px] ${msg.sender === 'firm' ? 'text-blue-200' : 'text-content-muted'}`}>
                    <span>{msg.time}</span>
          ...
          <div className="border-t border-surface-border bg-surface-raised p-3">
            <div className="flex items-end gap-2">
              <button className="rounded-md p-2 text-content-muted hover:bg-surface-sunken"><Paperclip className="h-4 w-4" /></button>
              <button className="rounded-md p-2 text-content-muted hover:bg-surface-sunken"><Image className="h-4 w-4" /></button>
              <input
                value={messageInput}
                onChange={(e) => setMessageInput(e.target.value)}
                placeholder="Type a message..."
                className="flex-1 rounded-lg border border-surface-border bg-surface px-3 py-2.5 text-[13px] placeholder:text-content-muted focus:border-blue-500 focus:bg-surface-raised focus:outline-none"
              />
              <button className="grid h-10 w-10 place-items-center rounded-lg bg-blue-600 text-white hover:bg-blue-500">
                <Send className="h-4 w-4" />
              </button>
            </div>
            <div className="mt-2 flex items-center justify-between text-[12px] text-content-muted">
