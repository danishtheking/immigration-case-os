...
const STATUS_MAP = {
  draft: { label: 'Draft', color: 'bg-surface-sunken text-content-secondary' },
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-content">Forms Engine</h1>
            <p className="text-[12px] text-content-tertiary">{FORMS.length} forms across all active cases · auto-fill from case data</p>
          </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input placeholder="Search forms by code, case, or status..." className="w-full rounded-lg border border-surface-border bg-surface-raised py-2 pl-9 pr-4 text-[13px] placeholder:text-content-muted focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-200" />
          </div>
        </div>
        ...
        <div className="mt-4 space-y-2">
          {FORMS.map((form, i) => {
            ...
            return (
              <div key={i} className="group flex items-center gap-4 rounded-xl border border-surface-border bg-surface-raised p-4 transition-all hover:border-surface-border-hover hover:shadow-sm">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="text-[14px] font-semibold text-content">Form {form.code}</p>
                    <span className={`rounded-md px-2 py-0.5 text-[12px] font-semibold ${s.color}`}>{s.label}</span>
                    {form.warnings > 0 && (
                      <span className="flex items-center gap-0.5 rounded-md bg-amber-50 px-1.5 py-0.5 text-[12px] font-semibold text-amber-700">
                        <AlertTriangle className="h-2.5 w-2.5" /> {form.warnings}
                      </span>
                    )}
                  </div>
                  <p className="mt-0.5 text-[12px] text-content-muted">{form.case} · ed. {form.edition}</p>
                </div>

                {/* Progress */}
                <div className="w-40 shrink-0">
                  <div className="flex items-center justify-between text-[12px]">
                    <span className="text-content-tertiary">{form.filled}/{form.total} fields</span>
                ...
                <div className="flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                  <button className="rounded-md p-1.5 text-content-muted hover:bg-surface-sunken hover:text-content-secondary" title="Preview"><Eye className="h-4 w-4" /></button>
                  {form.rendered && <button className="rounded-md p-1.5 text-content-muted hover:bg-surface-sunken hover:text-content-secondary" title="Download PDF"><Download className="h-4 w-4" /></button>}
                  {!form.rendered && pct === 100 && <button className="rounded-md bg-content px-2.5 py-1 text-[12px] font-medium text-white hover:bg-content/90" title="Render PDF"><Printer className="mr-1 inline h-3 w-3" />Render</button>}
        ...
        <p className="mt-6 text-center text-[12px] text-content-muted">Preview mode · 10 supported forms · PDF auto-fill from case data · edition tracking</p>
      </main>
    </>
  );
}
