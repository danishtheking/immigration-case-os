...
export default async function SignUpPage(): Promise<ReactElement> {
  if (!isClerkConfigured()) {
    return <PreviewStub action="Create your firm" />;
  }
    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-surface-raised p-8 shadow-sm">
        ...
        <div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <b>Preview mode</b> — auth is bypassed. The real Clerk sign-up widget does not render because no Clerk keys are configured.
        </div>
        <p className="mt-4 text-sm text-slate-600">
          To enable real sign-up, either clear the Clerk keys in
